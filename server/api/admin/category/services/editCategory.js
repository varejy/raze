...
export default function editCategory (req, res) {
    const { name, path, positionIndex, hidden, id, filters, metaTitle, metaDescription, keywords } = req.body;
    ...
    getCategory(id)
        .then(oldCategory => {
            editCategoryQuery({ name, hidden, positionIndex, path, id, filters: filtersWithIds, metaTitle, metaDescription, keywords })
                .then(() => {
